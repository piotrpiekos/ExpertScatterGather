...
def create_kernels(): # it is necessary to construct kernels in this way in order to be compatible with torch.compile() in multigpu setting
    global expert_gather_backward_dX_kernel, expert_gather_backward_dW_kernel, expert_gather_forward_call
    ...
    @triton.autotune(
        ...
        key=['B', 'J', 'I', 'out_dtype_id', 'allow_tf32', 'dtype_id']
    )
    @triton.jit
...
def expert_gather_backward_call(
