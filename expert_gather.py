...
def get_cuda_autotune_config():
    return [
        triton.Config({'BLOCK_SIZE_B': 64, 'BLOCK_SIZE_J': 64, 'BLOCK_SIZE_I': 128, 'GROUP_SIZE_B': 8},
                      num_stages=3, num_warps=8),
        triton.Config({'BLOCK_SIZE_B': 32, 'BLOCK_SIZE_J': 64, 'BLOCK_SIZE_I': 128, 'GROUP_SIZE_B': 8},
                      num_stages=4, num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 64, 'BLOCK_SIZE_J': 32, 'BLOCK_SIZE_I': 128, 'GROUP_SIZE_B': 8},
                      num_stages=4, num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 64, 'BLOCK_SIZE_J': 64, 'BLOCK_SIZE_I': 64, 'GROUP_SIZE_B': 8},
                      num_stages=4, num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 64, 'BLOCK_SIZE_J': 64, 'BLOCK_SIZE_I': 256, 'GROUP_SIZE_B': 8},
                      num_stages=3, num_warps=8),
    ]

def get_old_cuda_autotune_config():
    return [
        triton.Config({'BLOCK_SIZE_B': 128, 'BLOCK_SIZE_J': 256, 'BLOCK_SIZE_I': 64, 'GROUP_SIZE_B': 8}, num_stages=3,
                      num_warps=8),
        triton.Config({'BLOCK_SIZE_B': 64, 'BLOCK_SIZE_J': 256, 'BLOCK_SIZE_I': 32, 'GROUP_SIZE_B': 8}, num_stages=4,
                      num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 128, 'BLOCK_SIZE_J': 128, 'BLOCK_SIZE_I': 32, 'GROUP_SIZE_B': 8}, num_stages=4,
                      num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 128, 'BLOCK_SIZE_J': 64, 'BLOCK_SIZE_I': 32, 'GROUP_SIZE_B': 8}, num_stages=4,
                      num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 64, 'BLOCK_SIZE_J': 128, 'BLOCK_SIZE_I': 32, 'GROUP_SIZE_B': 8}, num_stages=4,
                      num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 128, 'BLOCK_SIZE_J': 32, 'BLOCK_SIZE_I': 32, 'GROUP_SIZE_B': 8}, num_stages=4,
                      num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 64, 'BLOCK_SIZE_J': 32, 'BLOCK_SIZE_I': 32, 'GROUP_SIZE_B': 8}, num_stages=5,
                      num_warps=2),
        triton.Config({'BLOCK_SIZE_B': 32, 'BLOCK_SIZE_J': 64, 'BLOCK_SIZE_I': 32, 'GROUP_SIZE_B': 8}, num_stages=5,
                      num_warps=2),
        # Good config for fp8 inputs.
        triton.Config({'BLOCK_SIZE_B': 128, 'BLOCK_SIZE_J': 256, 'BLOCK_SIZE_I': 128, 'GROUP_SIZE_B': 8}, num_stages=3,
                      num_warps=8),
        triton.Config({'BLOCK_SIZE_B': 256, 'BLOCK_SIZE_J': 128, 'BLOCK_SIZE_I': 128, 'GROUP_SIZE_B': 8}, num_stages=3,
                      num_warps=8),
        triton.Config({'BLOCK_SIZE_B': 256, 'BLOCK_SIZE_J': 64, 'BLOCK_SIZE_I': 128, 'GROUP_SIZE_B': 8}, num_stages=4,
                      num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 64, 'BLOCK_SIZE_J': 256, 'BLOCK_SIZE_I': 128, 'GROUP_SIZE_B': 8}, num_stages=4,
                      num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 128, 'BLOCK_SIZE_J': 128, 'BLOCK_SIZE_I': 128, 'GROUP_SIZE_B': 8}, num_stages=4,
                      num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 128, 'BLOCK_SIZE_J': 64, 'BLOCK_SIZE_I': 64, 'GROUP_SIZE_B': 8}, num_stages=4,
                      num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 64, 'BLOCK_SIZE_J': 128, 'BLOCK_SIZE_I': 64, 'GROUP_SIZE_B': 8}, num_stages=4,
                      num_warps=4),
        triton.Config({'BLOCK_SIZE_B': 128, 'BLOCK_SIZE_J': 32, 'BLOCK_SIZE_I': 64, 'GROUP_SIZE_B': 8}, num_stages=4,
                      num_warps=4)
    ]
...
def get_hip_autotune_config():
    return [
        triton.Config(
            {'BLOCK_SIZE_B': 128, 'BLOCK_SIZE_J': 256, 'BLOCK_SIZE_I': 16, 'GROUP_SIZE_B': 1, 'waves_per_eu': 2},
            num_warps=4, num_stages=2),
        triton.Config(
            {'BLOCK_SIZE_B': 256, 'BLOCK_SIZE_J': 256, 'BLOCK_SIZE_I': 16, 'GROUP_SIZE_B': 4, 'waves_per_eu': 2},
            num_warps=8, num_stages=2),
        triton.Config(
            {'BLOCK_SIZE_B': 128, 'BLOCK_SIZE_J': 128, 'BLOCK_SIZE_I': 32, 'GROUP_SIZE_B': 1, 'waves_per_eu': 2},
            num_warps=8, num_stages=2),
        triton.Config(
            {'BLOCK_SIZE_B': 64, 'BLOCK_SIZE_J': 128, 'BLOCK_SIZE_I': 32, 'GROUP_SIZE_B': 8, 'waves_per_eu': 3},
            num_warps=4, num_stages=2),
        triton.Config(
            {'BLOCK_SIZE_B': 64, 'BLOCK_SIZE_J': 64, 'BLOCK_SIZE_I': 32, 'GROUP_SIZE_B': 1, 'waves_per_eu': 8},
            num_warps=4, num_stages=2),
    ]
...
class ExpertGatherFunction(torch.autograd.Function):
    ...
    """
    @staticmethod
    def forward(ctx, X, W, Ind, out_type=torch.float32, allow_tf32=False):
        """
        ...
        expert_gather_forward[grid](
            ...
            out_dtype_id=dtype_to_type_id(out_type), allow_tf32=allow_tf32, dtype_id=dtype_to_type_id(out_type)
        )
    ...
    def backward(ctx, grad_output):
        ...
        expert_gather_backward_dX_kernel[grid_dX](
            ...
            out_dtype_id=dtype_to_type_id(ctx.dtype), allow_tf32=ctx.allow_tf32, dtype_id=dtype_to_type_id(ctx.dtype)
        )
        ...
        return grad_X, grad_W, None, None, None  # The third return is for the activation parameter
