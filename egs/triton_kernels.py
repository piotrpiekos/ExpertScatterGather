...
# create_kernels() is necessary to operate in multigpu scenario with torch.compile
def create_kernels():
...
class ExpertGatherFunction(torch.autograd.Function):
    ...
    """
    @staticmethod
    def forward(ctx, X: torch.Tensor, W: torch.Tensor, Ind: torch.Tensor, out_type=torch.float32):
        """
...
class ExpertScatterFunction(torch.autograd.Function):
    ...
    """
    @staticmethod
    def forward(ctx, Y: torch.Tensor, W: torch.Tensor, Ind: torch.Tensor, T: int, out_type=torch.float32):
        """
